...
class Components(object):
    ...
    def get_center_distance(self):
        '''
            Get distance from component mean to latent component mean
    ...
    def get_cov_dist(self, norm='F'):
        '''
            Get distance from component covariance matrix to latent
            covariance matrix for each component in each sample.
    ...
    def get_latent_bhattacharyya_overlap_quotient(self):
        ...
        for suco in self.mergeind:
            for k in suco:
                otherind = np.array([not (kk in suco) for kk in range(self.K)])
                for j in range(self.J):
                    corrdist = bhat.bhattacharyya_overlap(self.mupers[j, k, :], self.Sigmapers[j, k, :, :], self.mulat[k, :], self.Sigmalat[k, :, :])
                    wrongdist = max([bhat.bhattacharyya_overlap(self.mupers[j, k, :], self.Sigmapers[j, k, :, :], self.mulat[kk, :], self.Sigmalat[kk, :, :]) for kk in otherind])
