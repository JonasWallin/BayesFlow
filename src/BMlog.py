...
class BMresult(object):
    ...
    def savesim(self,hGMM):
        ...
        if MPI.COMM_WORLD.Get_rank() == 0:
            ...
            self.add_Sigmaexp(Qs,nus)
            self.add_mupers(mus)
            self.add_Sigmapers(Sigmas)
            self.add_prob(ps)            

    def postproc_burn(self):
        '''
...
class BMEresult(BMresult):

    '''
        Class for saving results from sampling of posterior distribution
        NB! Does save classification frequencies and sample names.
    '''
    
    def __init__(self,hGMM,sim,savesamp = [],nbrsave=None,nbrsavey=None):
        super(BMEresult,self).__init__(hGMM,sim,savesamp,nbrsave,nbrsavey)
        self.batch = 1000
        self.ii = -1
    ...
    def savesim(self,hGMM):
        ...
        '''
        super(BMEresult,self).savesim(hGMM)
        self.ii += 1
